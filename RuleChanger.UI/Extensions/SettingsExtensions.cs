...
using RuleChanger.Core.Helpers;
using RuleChanger.Core.Models;

namespace RuleChanger.UI.Extensions;
...
public static class SettingsExtensions
{
    public static bool SaveSettings(this Control formControl)
    {
        ...
        {
            MessageBox.Show($"SettingsExtensions: {ex.Message}");
            return false;
        }
    }
    ...
    public static bool ReadSettings(this Control formControl)
    {
        try
        {
            ...
            if (!StorageHelper.Exists(settingsFile)) return false;

            var json = StorageHelper.Read(settingsFile);
            if (json == null) return false;

            var data = JsonConvert.DeserializeObject<List<FormSettingModel>>(json);
    ...
    private static List<FormSettingValueModel> GetTreeViewSettings(TreeNodeCollection nodes)
    {
        ...
        foreach (TreeNode node in nodes)
            result.Add(new FormSettingValueModel()
            {
                ...
                Children = node.Nodes.Count > 0 ? GetTreeViewSettings(node.Nodes) : default!
            });
